...
function toHealthState(score) {
    if (score >= HEALTH_THRESHOLDS.good)
        return HEALTH_LABELS.good;
    if (score >= HEALTH_THRESHOLDS.warning)
        return HEALTH_LABELS.warning;
...
function computeFailureStreak(issues) {
    ...
    for (const issue of ordered) {
        if (!FAILED_SIGNAL_STATUSES.has(issue.status))
            break;
        streak += 1;
    }
    return streak;
...
export async function buildGardenSnapshot(context, input) {
