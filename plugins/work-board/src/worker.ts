...
function buildColumnsFromMissions(missions: MissionCard[]): MissionColumn[] {
...
function deriveSpawnGraph(graph: MissionIssueGraphSnapshot): MissionIssueGraphSnapshot {
  const nodeById = new Map(graph.graph.nodes.map((node) => [node.id, node] as const));
  const seedIssueIds = graph.seedIssueIds.filter((issueId) => nodeById.has(issueId));
  const issueReachable = new Set<string>(seedIssueIds);
  ...
  const reachableNodes = new Map<string, GraphNode>();
...
export function buildWorkBoardSnapshot(
  ...
  const columns: MissionColumn[] = buildColumnsFromMissions(missions);
  ...
  const missionsByProjectId = new Map<string | null, MissionCard[]>();
  for (const mission of missions) {
    const rootIssue = issueById.get(mission.missionId) ?? null;
    const projectId = asString((rootIssue as unknown as JsonRecord)?.projectId) || null;
  ...
  const projectGroups: ProjectGroupData[] = [];
...
async function listAllAgents(ctx: PluginContext, companyId: string): Promise<AgentRecord[]> {
...
async function listAllIssues(ctx: PluginContext, companyId: string): Promise<IssueRecord[]> {
  const limit = 250;
...
async function listIssueComments(
  ...
  try {
    ...
    const comments = (await res.json()) as Array<{ body?: unknown; companyId?: unknown }>;
...
async function loadBoardSnapshot(ctx: PluginContext, companyId: string): Promise<WorkBoardSnapshot> {
  ...
  try {
    ...
    if (res.ok) {
      const raw = (await res.json()) as Array<{ id?: unknown; name?: unknown }>;
      projects = raw
        .map((project) => ({
