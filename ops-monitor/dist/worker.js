...
async function wakeupAgent(agentId, issueId, reason, forceFreshSession = false) {
...
function isIdleAgent(agent) {
    return agent.status !== "running" && agent.status !== "paused";
}
function isInspectorAgent(agent) {
...
function hoursAgoText(updatedAt) {
...
function getStaleThresholdMs(value) {
...
function getNestedRecord(root, key) {
    const value = root[key];
    return value && typeof value === "object" ? value : {};
}
async function readConfig(ctx) {
    const raw = await ctx.config.get();
    const targetCompanyNames = new Set(toStringArray(raw?.targetCompanyNames));
    const record = raw ?? {};
...
async function resolveTargetCompanies(ctx, config) {
    const companies = await ctx.companies.list();
    return companies.filter((company) => config.targetCompanyNames.has(company.name));
}
async function listIssuesByStatus(ctx, companyId, status) {
    return ctx.issues.list({ companyId, status, limit: 500, offset: 0 });
}
async function wakeStuckTodosForCompany(ctx, company) {
...
async function wakeInReviewForCompany(ctx, company) {
    const [issues, agents] = await Promise.all([
...
async function wakeStuckJob(ctx) {
    const config = await readConfig(ctx);
    if (!config.enableWakeStuck && !config.enableInReviewWake)
        return;
...
async function collectStuckAndReview(ctx, company, config) {
...
async function collectErrorAgents(ctx, company) {
...
async function dailySummaryJob(ctx) {
    const config = await readConfig(ctx);
...
const plugin = definePlugin({
    async setup(ctx) {
        ctx.jobs.register(JOB_KEYS.wakeStuck, async (_jobCtx) => {
            await wakeStuckJob(ctx);
        });
        ctx.jobs.register(JOB_KEYS.dailySummary, async (_jobCtx) => {
